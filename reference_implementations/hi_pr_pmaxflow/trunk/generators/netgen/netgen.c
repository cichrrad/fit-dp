 ...
 ***/
  ...
  for (i = NODES-SOURCES-SINKS; i > (4*(NODES-SOURCES-SINKS)+9)/10; i--) {
    node = choose_index(handle, (INDEX)randomm(1L, (long)index_size(handle)));
 ...
 ***/
  ...
  for (source = 1; source <= SOURCES; source++) {
    ...
    for (i = 0; i < sinks_per_source; i++) {
      sinks[i] = choose_index(handle, (INDEX)randomm(1L, (long)index_size(handle)));
    }
    ...
    for (i = 0; i < sinks_per_source; i++) {
      sort_count++;
      partial_supply = randomm(1L, (long)supply_per_sink);
      j = randomm(0L, (long)sinks_per_source - 1);
      tail[sort_count] = k;
      head[sort_count] = sinks[i] + 1;
      B[sinks[i]] -= partial_supply;
      B[sinks[j]] -= (supply_per_sink - partial_supply);
      k = source;
      for (j = randomm(1L, (long)chain_length); j > 0; j--)
	k = pred[k];
    }
	...
	cap = SUPPLY;
	if (randomm(1L, 100L) <= CAPACITATED)
	  cap = MAX(B[source-1], MINCAP);
	cost = MAXCOST;
	if (randomm(1L, 100L) > HICOST)
	  cost = randomm(MINCOST, MAXCOST);
...
CAPACITY supply;
{
  ...
  for (i = 0; i < sources; i++) {
    B[i] += (partial_supply = randomm(1L, (long)supply_per_source));
    B[randomm(0L, (long)(sources - 1))] += supply_per_source - partial_supply;
  }
  B[randomm(0L, (long)(sources - 1))] += supply % sources;
}
...
long parms[];
{
  ...
  for (source = 1; source <= NODES/2; source++) {
    index = choose_index(skeleton, (INDEX)randomm(1L, (long)index_size(skeleton)));
    SAVE_ARC(source, index, randomm(MINCOST, MAXCOST), 1);
...
NODE desired_tail;
{
  ...
  for ( ; limit > 0; limit--) {
    index = choose_index(handle, (INDEX)randomm(1L, (long)pseudo_size(handle)));
    cap = SUPPLY;
    if (randomm(1L, 100L) <= CAPACITATED)
      cap = randomm(MINCAP, MAXCAP);
    SAVE_ARC(desired_tail, index, randomm(MINCOST, MAXCOST), cap);
  }
}
